...
(function (root, factory) {
  ...
  /* The echo on English system lines: a short room, not a canyon. The wet path
     hangs beside the dry voice and is silent until a turn asks for it. */
  const ECHO_DELAY = 0.105;
  const ECHO_FEEDBACK = 0.24;
  const ECHO_DAMP = 3200;
  const ECHO_WET = 0.30;
  const ECHO_RAMP = 0.06;

  /* The timbre: JARVIS_VOICE_TIMBRE (0-100, from /api/config) decides how far a
     stock voice is turned into something that belongs on a bridge. The brain
     sends the same PCM either way; this is the browser's doing, so it costs no
     credit and no latency. Every value scales off that one number, so 0 is the
     dry voice. */
  const TIMBRE_HP = [120, 360];
  const TIMBRE_PEAK = 2600;
  const TIMBRE_PEAK_DB = 8;
  const TIMBRE_COMB = 0.0075;
  const TIMBRE_COMB_FB = 0.45;
  const TIMBRE_TAIL = 0.085;
  const TIMBRE_TAIL_FB = 0.35;
  const TIMBRE_TAIL_LP = 3500;
  const TIMBRE_WET = 0.55;
  const TIMBRE_RING = 42;
  const TIMBRE_RING_DEPTH = 0.25;
  const TIMBRE_RAMP = 0.08;

  /* What the brain accepts as the reason a window left the screen. */
  const GONE_REASONS = ['closed', 'timeout', 'next-turn', 'replaced', 'aged-out'];
...
`;
  ...
  function create(opts) {
    ...
    const voice = {
      ...
      echo: null, timbre: null, timbreAmount: 0,
    ...
    function route(m) {
      ...
      if (m.kind === 'ui_lang') {
        if (m.lang === 'nl' || m.lang === 'en') bus.emit('uiLang', m.lang);
        return;
      }
      if (m.kind === 'desk') {
    ...
    /* Closing a window throws away the only copy on screen. The brain keeps the
       contents, so telling it which one went is enough to be asked for it back
       in words. It answers nothing: the screen is reporting, not asking. */
    function displayClosed(id, reason) {
      if (!id || GONE_REASONS.indexOf(reason) === -1) return;
      send({ kind: 'display_closed', id: String(id), reason });
    }
    ...
    /* Always built and always in the dry path, wired for silence at 0: changing
       the graph while audio is scheduled on it clicks, moving numbers does not. */
    function buildTimbre(ctx) {
      ...
      try {
        ...
        /* Ring modulation: an oscillator writing into a gain, so the buzz rides
           under the voice instead of beside it. */
        ...
        /* The echo hung off the gain node before this chain was put in. */
    ...
    function setVoiceTimbre(amount) {
      const a = clamp(Number(amount) || 0, 0, 100) / 100;
      voice.timbreAmount = a * 100;
      const chain = voice.timbre;
      if (!chain || !voice.ctx) return;
      rampTo(chain.hp.frequency, TIMBRE_HP[0] + (TIMBRE_HP[1] - TIMBRE_HP[0]) * a, TIMBRE_RAMP);
      rampTo(chain.peak.gain, TIMBRE_PEAK_DB * a, TIMBRE_RAMP);
      rampTo(chain.dry.gain, 1 - 0.45 * a, TIMBRE_RAMP);
      rampTo(chain.wet.gain, TIMBRE_WET * a, TIMBRE_RAMP);
      rampTo(chain.combFb.gain, TIMBRE_COMB_FB * a, TIMBRE_RAMP);
      rampTo(chain.tailFb.gain, TIMBRE_TAIL_FB * a, TIMBRE_RAMP);
      rampTo(chain.depth.gain, TIMBRE_RING_DEPTH * a, TIMBRE_RAMP);
    }

    /* 'echo' for the English system lines, 'none' for everything else */
    function setVoiceFx(kind) {
      voice.fx = kind === 'echo' ? 'echo' : 'none';
      if (!voice.echo || !voice.ctx) return;
      rampTo(voice.echo.wet.gain, voice.fx === 'echo' ? ECHO_WET : 0, ECHO_RAMP);
    }

    /* The deployment decides how coloured the voice is, so the page asks. It
       arrives long before the first answer; if not, the chain sits at zero and
       the value is applied when it is built. */
    function loadConfig() {
      if (typeof fetch !== 'function') return;
      fetch('/api/config').then((r) => r.json()).then((cfg) => {
        const amount = Number(cfg && cfg.voiceTimbre);
        if (Number.isFinite(amount)) setVoiceTimbre(amount);
      }).catch(() => {});
    }
    ...
    if (opts.loadConfig !== false) loadConfig();
    ...
    const link = {
      ...
      onUiLang(fn) { return bus.on('uiLang', fn); },
