...
(function () {
  ...
  function renderAgenda(vm) {
    ...
    setMeta(
      'agenda',
      now.toLocaleDateString(uiLocale(), { weekday: 'short', day: 'numeric', month: 'short' })
    );
    if (!vm) {
  ...
  function dismissFreeCard(id, reason) {
    ...
    if (!card) return;
    /* 'cleared' came from the brain itself; everything else it is told about. */
    if (reason !== 'cleared' && link && typeof link.displayClosed === 'function') {
      link.displayClosed(id, reason);
    }
    if (card._timer) clearTimeout(card._timer);
    delete freeCards[id];
    releaseMedia(card);
  ...
  function wire(l) {
    ...
    /* The screen has a language of its own, switched by asking JARVIS. */
    l.onUiLang(function (lang) {
      if (window.JarvisI18n) JarvisI18n.set(lang);
    });
    /* Dates inside panels were drawn in the old language; draw them again. */
    window.addEventListener('jarvis:ui-lang', hydrateFromCache);

    l.on('ready', function () { greetOnArrival(l); });
  ...
  function wireAsk(l) {
    ...
    focusAsk();
    setTimeout(focusAsk, 1500);
    l.on('ready', focusAsk);
  }

  /* ---------- Greeting on arrival ----------
     Open the page with ?greet=1 and JARVIS says hello by himself: the `say`
     path, straight to the voice, no model asked. Picked at random so unlocking
     twice in a row does not sound like a recording. The window stays open
     across a screen lock, so coming back after a minute away is an arrival
     too; a reconnect after a dropped socket is not. */
  const GREETINGS = [
  ...
  function greetOnArrival(l) {
    if (!WANTS_GREETING || greeted || !l) return;
    greeted = true;
    setTimeout(function () {
      if (l.mode !== 'idle') return;
      l.say(GREETINGS[Math.floor(Math.random() * GREETINGS.length)], 'en');
    }, 600);
  }
  ...
  document.addEventListener('visibilitychange', function () { document.hidden ? markAway() : markBack(); });
